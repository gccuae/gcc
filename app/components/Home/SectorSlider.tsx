    ...
    <section className="wrapper pt-47px pb-0 xl:pb-47px bg-black dark:bg-[#191919] text-white overflow-hidden">
      <div className="container">
        <motion.h2 variants={moveUp(0)} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.2 }}
         className="text-5xl font-normal leading-[1.147058823529412] pb-4 xl:pb-27px ">{data.title}</motion.h2>
        <div>
          <div className="relative">
            ...
            <Swiper className="sector-slider "
              slidesPerView={2}
              spaceBetween={0}
              // loop={true}
            ...
            >
              {data.items.map((item, index) => (
                <SwiperSlide key={index} className="border-t-1 border-r-1 border-foreground relative group pr-[15px] pl-[15px] xl:pr-0 xl:pl-0">
                  ...
                  <motion.div variants={moveUp(index * 0.2)} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.2 }} className="flex flex-col justify-between  xl:max-h-[30em] overflow-hidden z-40 relative group">
                    <div className="flex items-center justify-between mb-3 xl:mb-[25px] xl:pl-4 xl:pr-3 pt-6 xl:pt-[37px] group-hover:xl:pl-[37px]  group-hover:xl:pt-[35px] group-hover:xl:pr-[35px] group-first:pl-0 transition-all duration-300">
                      <div className="flex items-center gap-2 xl:gap-4">
                        ...
                        <h3 className="text-2xl leading-[1.5625] font-normal transition-all duration-300">{item.title}</h3>
                      </div>
                    ...
                    <div className="xl:pl-4 xl:pr-3 pt-6 group-hover:xl:pl-[37px]  group-hover:xl:pt-[35px] group-hover:xl:pr-[35px] group-first:pl-0 transition-all duration-300">
                      <p className="text-lg font-[300] leading-[1.526315789473684] pb-5 xl:pb-0 xl:opacity-0 xl:h-0 group-hover:xl:h-auto group-hover:xl:opacity-100 transition-all ease-in-out duration-300 group-hover:xl:pb-[23px] sector-description relative z-10 dark:text-white/80">{item.description}</p>
                    </div>
