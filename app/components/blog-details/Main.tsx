...
const Main = ({ title, category, date, author, mainImage, description, extraContent, quote }: Props) => {
  return (
    <section className="py-57px dark:bg-black">
      <div className="container">
        ...
        <div className="mb-27px">
          <Image src={mainImage} alt="blog-details" width={1920} height={1280} className="w-full" />
        </div>
        <div className="mb-47px">
          {description.map((para, index) => (
            <p key={index} className="mb-5 xl:mb-5 text-lg leading-lh-text19 font-normal dark:text-white">{para}</p>
          ))}
        </div>
        <div className="mb-47px">
          <h3 className="text-2xl leading-lh-text32 font-normal mb-5 xl:mb-[27px] dark:text-white">{extraContent.title}</h3>
          <ul className="square-list list-inside space-y-3 xl:space-y-4 dark:text-white">
            {extraContent.description.map((item, index) => (
              <li key={index} className="text-lg leading-lh-text19 font-normal fong-light">{item}</li>
            ))}
          </ul>
        </div>
        <div>
          <div className="bg-[#1c1c1c] border-l-6 border-primary dark:border-accent max-w-[938px] pl-6 pr-6 py-6 md:pl-14 md:pr-14 xl:pl-[71px] xl:pr-[72px]  md:py-15 xl:py-[81px] relative">
            ...
            <div className="flex items-center justify-between">
              ...
              <Image src={assets.quote} alt="quote" width={50} height={50} className="w-10 h-10" />
            </div>
            <div className="absolute bottom-0 left-0 w-full h-[25px]">
              <Image src={assets.scale} alt="scale" width={1920} height={25} className="w-full object-fill" />
            </div>
          </div>
        </div>
      </div>
