                ...
                >
                    ...
                    <motion.div variants={moveUp(0.6)} initial="hidden" whileInView="show">
                        <Link
                            ...
                            className={!data?.buttonLink?.trim() ? "pointer-events-none" : ""}
                        >
                            <button
                                type="button"
                                disabled={!data?.buttonLink?.trim()}
                                className={`w-fit border border-[#7AC142] text-white px-[20px] xl:px-[30px] py-[11px] xl:py-[15px] rounded-full text-base uppercase transition-colors bg-white/10 backdrop-blur-md
      ${data?.buttonLink?.trim() ? "cursor-pointer hover:bg-[#7AC142] hover:text-black" : "cursor-default"}`}
                            >
                                {data.buttonTitle}
                            </button>
                        </Link>
