...
const WhatWeDo = ({ data, expertiseData }: WhatWeDoComponentProps) => {
  ...
  return (
    <section
      className={`bg-light-white dark:bg-light-dark overflow-hidden  ${data.hidden ? "hidden" : ""}`}
    >
      <div className="border-b dark:border-white/20 dark:border-white/20">
        <div className="container pt-37px pb-5 md:pt-47px lg:pt-47px xl:pt-57px  xl:pb-57px">
          <motion.h2
            ...
            whileInView="show"
            viewport={{ once: true }}
            className=" text-4xl 2xl:text-5xl leading-lh-title text-black dark:text-white mb-2 md:mb-4 lg:mb-[13px]"
          >
            {data.mainTitle}
          </motion.h2>
          <motion.h3
            ...
            whileInView="show"
            viewport={{ once: true }}
            className="md:text-xl xl:text-2xl leading-[1.2]  xl:leading-[1.5625] dark:text-white text-para-color"
          >
            {data.subTitle}
          </motion.h3>
        </div>
      ...
      <SpecialContainer className="pl-[15px]" side="right">
        <motion.div
          ...
          whileInView="show"
          viewport={{ once: true }}
          className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-[52%_48%] items-stretch"
        >
          <div className="md:pr-3 xl:pr-70px lg:border-r dark:border-white/20 dark:border-white/20 py-5 lg:pt-4 lg:pb-47px xl:pt-[86px] xl:pb-[90px] flex flex-col ">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-2 xl:gap-x-50px mb-7 xl:mb-14 ">
              {expertiseData.items.map((item, index) => (
                <Link key={index} href={`expertise/${item?.slug || "#"}`}>
                  <motion.button
                    ...
                    className={`flex items-center gap-2 w-full justify-between border-b dark:border-white/20  group cursor-pointer dark:text-white ${
                      index === activeIndex
