    ...
    <section className="bg-light-white dark:bg-[#0d0d0d] overflow-hidden">
      <div className="border-b border-smgray">
        <div className="container pt-57px pb-5 xl:py-57px">
          <h2 className="text-6xl leading-lh-title text-black dark:text-white mb-2">What We Do</h2>
          <h3 className="text-2xl leading-[1.2]  lg:leading-[1.5625] dark:text-white">Unlocking Potential Through Construction Excellence</h3>
        </div>
      </div>
      <SpecialContainer className="pl-[15px]" side="right">
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-[52%_48%] ">
          <div className="md:pr-3 xl:pr-70px md:border-r border-smgray py-5 lg:py-4 xl:pt-[86px] xl:pb-[90px] flex flex-col justify-between">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-2 xl:gap-x-50px mb-4 xl:mb-10 ">
              {aboutData.whatWeDo.items.map((item, index) => (
                <button key={index} onClick={() => setActiveIndex(index)} className={`flex items-center gap-2 w-full justify-between border-b border-smgray group cursor-pointer dark:text-white ${index === activeIndex ? 'text-black dark:text-primary' : ''}`}>
                  <p className="text-xl leading-[1.2] py-3 md:py-5 text-left group-hover:text-black transition-all duration-300 dark:group-hover:text-primary ">{item.title}</p>
          ...
          <div className="pt-2 pb-10 md:py-5 md:pl-5 xl:py-10 xl:pl-10 relative overflow-hidden md:min-h-[800px] ">
            <AnimatePresence mode="wait">
              ...
              >
                <Image
                  ...
                  width={1500}
                  height={1500}
                  className="w-full   h-[400px] md:max-h-[700px] object-cover "
                />
              </motion.div>
            </AnimatePresence>
          </div>
