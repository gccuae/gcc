    ...
    <section className="bg-light-white dark:bg-[#0d0d0d] overflow-hidden">
      <div className="border-b border-smgray">
        <div className="container pt-57px pb-5 xl:py-57px">
          <h2 className="text-6xl leading-lh-title text-black dark:text-white mb-4 lg:mb-2">What We Do</h2>
      ...
      <SpecialContainer className="pl-[15px]" side="right">
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-[52%_48%] ">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-2 xl:gap-x-50px mb-7 xl:mb-10 ">
              {aboutData.whatWeDo.items.map((item, index) => (
                <button key={index} onClick={() => setActiveIndex(index)} className={`flex items-center gap-2 w-full justify-between border-b border-smgray group cursor-pointer dark:text-white ${index === activeIndex ? 'text-black dark:text-primary' : ''}`}>
                  <p className="text-xl leading-[1.2] py-4 md:py-5 text-left group-hover:text-black transition-all duration-300 dark:group-hover:text-primary ">{item.title}</p>
