...
export const BlogItem = ({ item, index }: { item: { id: number; title: string; image: string; date: string; category: string }; index: number }) => {
  ...
  return (
    <div className="relative group border-b border-smgray hover:border-primary pb-2 md:pb-27px transition-all duration-300">
      <div className={`${index % 2 === 0 ? "h-[300px] xl:h-[415px]" : "h-[200px] xl:h-[280px]"} p-3 xl:p-5 overflow-hidden group/img relative`}
        ...
        >
          <Link href={`/blog/${item.title}`} rel="noopener noreferrer" className="bg-white rounded-full w-6 h-6 flex items-center justify-center xl:w-20 xl:h-20 pointer-events-auto transition-transform duration-200 hover:scale-110 group-hover:opacity-100 ">
      ...
      <div className="mt-5 xl:mt-[27px] relative">
        ...
        <h3 className="text-xl leading-normal font-normal mb-3 xl:mb-[12px] text-black dark:text-white">{item.title}</h3>
      </div>
    </div>
  );
};