...
const KeyProjects = () => {
  ...
  const handlePrevClick = () => {
    ...
    if (swiperRef.current) {
      console.log("Current slide index before prev:", swiperRef.current.realIndex);
      swiperRef.current.slidePrev();
      setTimeout(() => {
        console.log("Current slide index after prev:", swiperRef.current?.realIndex);
      }, 100);
    }
  };
  ...
  const handleNextClick = () => {
    ...
    if (swiperRef.current) {
      console.log("Current slide index before next:", swiperRef.current.realIndex);
      swiperRef.current.slideNext();
      setTimeout(() => {
        console.log("Current slide index after next:", swiperRef.current?.realIndex);
      }, 100);
    }
  };
    ...
    <section className="py-57px xl:pb-25 bg-light-white dark:bg-black">
      <div className="container">
        <div className="mb-57px flex items-center justify-between">
          <h2 className="text-5xl leading-lh-text68 font-normal text-black dark:text-white"> Key Projects </h2>
          <div className="flex border border-foreground dark:border-white rounded-full">
      ...
      >
        {serviceDetailsData.keyProjects.items.map((item, index) => (
          <SwiperSlide key={index}>
            <div className="relative h-[300px] xl:h-[633px] flex flex-col justify-end px-[30px] py-6 xl:py-[35px] group">
              <Image src={item.img} alt={item.title} width={500} height={500} className="w-full h-full object-cover absolute inset-0 z-0" />
              <div className="relative z-10 opacity-0 group-hover:opacity-100 content-box  transition-all duration-300">
                <div className="bg-black w-fit px-5 py-3">
                  <p className="text-lg leading-lh-text19 font-normal text-white">{item.info}</p>
                </div>
                <div className="bg-light-white dark:bg-black w-fit p-4 min-w-[60%] flex items-center justify-between gap-4 group">
                  <h3 className="text-2xl leading-normal font-normal text-black dark:text-white">{item.title}</h3>
                  <Link href={"#"} className="bg-black w-10 h-10 xl:w-15 xl:h-15 rounded-full flex items-center justify-center">
                    <Image src={assets.linkArrowGreen} alt="arrow-right" width={20} height={20} className="w-5 h-5 xl:w-[19px] xl:h-[19.05px] -translate-x-2 translate-y-2 transition-all duration-300 group-hover:translate-y-0 group-hover:-translate-x-0" />
                  </Link>
                </div>
              </div>
            </div>
      ...
      <div className="swiper-pagination w-full">
        <div className="flex justify-center items-center gap-2 mt-6 w-fit mx-auto">
          {serviceDetailsData.keyProjects.items.map((_, idx) => (
            <button key={idx} onClick={() => swiperRef.current?.slideToLoop(idx)} className={`w-3 h-[3px] rounded-full transition ${swiperRef.current?.realIndex === idx ? "bg-accent w-[27px]" : "bg-mdgray"}`}></button>
          ))}
        </div>
      </div>
    </section>
