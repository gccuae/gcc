        ...
        {projects.map((item, index) => (
          ...
          >
            <motion.div
              ...
              whileInView="show"
              viewport={{ once: true }}
              className="relative h-[350px] xl:h-[450px] 2xl:h-[633px] w-full lg:w-[700px] 2xl:w-[1003px] flex flex-col justify-end px-6 py-6 xl:py-8 group"
            >
              <Image
                src={item.thumbnail}
                alt={item.title}
        ...
        <div className="flex justify-center items-center gap-2 mt-4 lg:mt-[40px] w-fit mx-auto">
          {projects.map((_, idx) => (
            <button
              key={idx}
              onClick={() => swiperRef.current?.slideToLoop(idx)}
              className={`w-3 h-[3px] rounded-full transition-all duration-300 ${
