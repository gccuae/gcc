...
const ExpertiseCard = ({
  ...
  return (
    <div
      key={index}
      className="relative border-b border-smgray pb-3 lg:pb-4 xl:pb-[25px] group"
    >
      <Link href={`/expertise/${item.title}`}>
        ...
        <div className="grid grid-cols-[auto_1fr] items-center gap-3 xl:gap-[25.29px]">
          ...
          <h3 className="text-xl lg:text-2xl leading-lh-text32 dark:text-white dark:group-hover:text-primary group-hover:text-black transition-all duration-300 ">
            {item.title}
          </h3>
        </div>
      </Link>
