  ...
  return (
    <section className="py-37px md:py-47px xl:py-57px bg-white dark:bg-black">
      <div className="container">
        {/* Title */}
        <motion.h2 variants={moveUp()} initial="hidden" whileInView="show" viewport={{ once: true }} className=" text-4xl 2xl:text-5xl md:text-6xl font-light leading-lh-title text-black dark:text-white mb-3 md:mb-27px" >
          {title}
        </motion.h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 md:gap-30px gap-20px">
          {matters.map((item, index) => (
            <motion.div
              ...
              key={index}
              className="group cursor-pointer flex flex-col overflow-hidden md:border-b border-gray-200 dark:border-white/50 hover:border-primary transition-colors duration-300 pb-27px xl:pb-[31px]"
            >
              {/* Thumbnail */}
