...
const ProjectCard = ({ item, index }: { item: Project; index: number }) => {
    ...
    return (
        ...
        <div
            key={index}
            className="relative border-b border-smgray hover:border-primary pb-[15px] lg:pb-27px group transition-all duration-300 flex flex-col h-full"
        >
            <div
            ...
            <div className="mt-5 xl:mt-[27px] relative">
                <div className="flex justify-between items-center mb-2 xl:mb-[12px]">
                    <div className="flex items-center flex-wrap gap-[7px] xl:gap-[13px] text-lg leading-[1] font-light dark:text-white text-para-color">
                        {item?.secondSection?.projectType?.name && <span>{item?.secondSection?.projectType?.name}</span>}
                        {item?.secondSection?.projectType?.name && <span className="text-black">|</span>}
                        {item?.secondSection?.sector?.name && <span>{item?.secondSection?.sector?.name}</span>}
                        {item?.secondSection?.sector?.name && <span className="text-black">|</span>}
                        {item?.secondSection?.location?.name && <span>{item?.secondSection?.location?.name}</span>}
                        {item?.secondSection?.location?.name && <span className="text-black">|</span>}
