    ...
    const animate = () => {
      ...
      setCursorStyle((prev) => {
        if (Math.abs(prev.x - newX) < 0.5 && Math.abs(prev.y - newY) < 0.5)
          return prev;
        return { x: newX, y: newY, visible: true };
      });
    ...
    >
      ...
      >
        ...
        {item.thumbnail ? (
          ...
          <div className="w-full h-full object-cover absolute top-0 left-0   group-hover:blur-[4px] group-hover:backdrop-blur-xl transition-all duration-300 bg-gray-200 flex items-center justify-center">
            350*350
          </div>
        )}
        <div className="absolute bottom-0 left-0 hidden lg:block w-full h-0 bg-black opacity-25 group-hover:h-full transition-all duration-300"></div>
        <div
          ...
          style={{
            ...
            scale: cursorStyle.visible ? 1 : 0.8,
          }}
        >
          <Link
            href={`/projects/${item.slug}`}
          ...
          >
            <Image
              ...
              width={20}
              height={20}
              className="w-5 h-5 object-contain xl:w-[19px] xl:h-[19px]"
            />
          </Link>
        </div>
      </div>
      <div className="mt-5 xl:mt-[27px] relative">
        <div className="flex justify-between items-center mb-4 xl:mb-[12px]">
          <div className="flex items-center flex-wrap gap-[7px] xl:gap-[13px] 2xl:text-lg leading-[1] font-light dark:text-white/80 text-para-color">
            ...
            {item?.secondSection?.projectType?.name && (
              <span className="text-black dark:text-secondary">|</span>
            )}
