...
const BtnPrimary = ({link, text, bgtrans, borderwight}: {link: string, text: string, bgtrans?: boolean, borderwight?:boolean}) => {
  return ( 
    <Link href={link} className={`${bgtrans ? "bg-transparent" : "bg-white"}
     hover:bg-mdgray hover:text-white flex items-center justify-center py-1 xl:py-[7.39px] px-4 xl:px-[28px] gap-2 transition-all
     duration-300 ease-in-out group border ${borderwight ? "border-white text-white" : "border-foreground"} dark:border-white rounded-4xl w-fit hover:shadow-xl dark:bg-[#0d0d0d]`}>
