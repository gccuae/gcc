...
const SidebarContent = () => {
  return (
    <aside>
          ...
          <li className="bg-black rounded-full w-8 h-8 xl:w-[41px] xl:h-[41px] flex items-center justify-center hover:translate-y-[-2px] hover:bg-accent transition-all duration-300 ease-in-out cursor-pointer active:bg-accent active:translate-y-0">
            <Link href="#">
              <svg width="14" height="15" viewBox="0 0 14 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M8.33097 6.35146L13.5437 0H12.3092L7.78395 5.51421L4.16841 0H0L5.4672 8.33879L0 15H1.23443L6.01421 9.17604L9.83159 15H14L8.33097 6.35146ZM6.6402 8.41239L6.08734 7.58127L1.67906 0.975261H3.57752L7.13456 6.30853L7.68742 7.13964L12.3092 14.0707H10.4108L6.63728 8.41239H6.6402Z" fill="white" />
              </svg>
            </Link>
          </li>
        </ul>
      ...
      <div>
        <h3 className="text-2xl leading-lh-text32 font-light mb-5 xl:mb-27px text-black dark:text-white">Related News</h3>
        <div>
          {newsDetails.relatedNews.map((item, index) => (
            <div key={index} className="mb-5 xl:mb-[17px] border-b border-smgray pb-5 xl:pb-[17px]">
              <Link href={`/news/${item.slug}`}>
                <Image src={item.image} alt={item.title} width={700} height={700} className="w-full h-50 object-cover" />
                <h4 className="text-lg leading-lh-text19 font-light text-black dark:text-white mt-3 xl:mt-[17px]">{item.title}</h4>
              </Link>
              <div className="flex mt-3 items-center justify-between">
                <span className="font-light text-base text-black dark:text-white">{item.sector}</span>
                <span className="font-light text-base">{item.date}</span>
              </div>
            </div>
          ))}
        </div>
        <button className="px-6 py-2 bg-light-white text-black rounded-3xl border border-mdgray uppercase flex items-center gap-2 transition" >
          <span>View All</span>
          <Image src={assets.singleGreenArrow} alt="arrow" width={20} height={20} className="inline" />
        </button>
      </div>
    </aside>
  );
